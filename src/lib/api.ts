...
function requireAdmin(env: Env, authHeader: string | undefined): boolean {
...
app.post('/scenarios/:slug/reviews', async (c) => {
  ...
  if (!body.content?.trim()) return c.json({ error: 'bad_request', message: '내용을 입력해주세요.' }, 400);
  if (body.content.length > 1000) return c.json({ error: 'bad_request', message: '후기는 1000자 이하로 작성해주세요.' }, 400);
...
app.post('/admin/scenarios', async (c) => {
  ...
  const { meta } = await c.env.DB.prepare(
    'INSERT INTO scenarios (slug, title, description, color, cover_r2) VALUES (?, ?, ?, ?, ?)',
  ).bind(body.slug, body.title, body.description ?? null, body.color, body.cover_r2 ?? null).run();
...
app.post('/admin/scenarios/:id/characters', async (c) => {
  ...
  ).bind(scenarioId, body.name, body.pl_name, body.image_r2 ?? null, body.bio ?? null, body.sort_order ?? 0).run();
  ...
  return c.json(await c.env.DB.prepare('SELECT * FROM characters WHERE id = ?').bind(charId).first(), 201);
});
...
app.put('/admin/characters/:id', async (c) => {
  ...
  await c.env.DB.prepare(`
    ...
    WHERE id = ?
  `).bind(body.name ?? null, body.pl_name ?? null, body.image_r2 ?? null, body.bio ?? null, body.sort_order ?? null, id).run();
...
app.post('/admin/upload', async (c) => {
  ...
  if (!key || !file) return c.json({ error: 'bad_request', message: 'key와 file은 필수입니다.' }, 400);
